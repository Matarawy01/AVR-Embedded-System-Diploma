...
void LCD_vinit()
{
	...
	DIO_vset_pindir('d',4,1);
	DIO_vset_pindir('d',5,1);
	DIO_vset_pindir('d',6,1);
...
{
	#if defined eight_bits_mode
	DIO_vwrite_port('d',data);
	DIO_vwrite_pin('B',RS,1);
	send_falling_edge();
	
	#elif defined four_bits_mode
	vwrite_highnibble('d',data>>4);
	DIO_vwrite_pin('B',RS,1);
	send_falling_edge();
	vwrite_highnibble('d',data);
	DIO_vwrite_pin('B',RS,1);
	send_falling_edge();
	_delay_ms(1);
	#endif
...
{
	#if defined eight_bits_mode
	DIO_vwrite_port('d',cmd);
	DIO_vwrite_pin('B',RS,0);
	send_falling_edge();
	
	#elif defined four_bits_mode
	vwrite_highnibble('d',cmd>>4);
	DIO_vwrite_pin('B',RS,0);
	send_falling_edge();
	vwrite_highnibble('d',cmd);
	DIO_vwrite_pin('B',RS,0);
	send_falling_edge();
	_delay_ms(1);
	#endif
